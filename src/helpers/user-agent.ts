...
export function getRandomYouTubeUserAgent() {
    const randomAndroidVersion =
        androidVersions[Math.floor(Math.random() * androidVersions.length)];
    const randomClientVersion =
        clientVersions[Math.floor(Math.random() * clientVersions.length)];
    const randomSdkVersion =
        sdkVersions[Math.floor(Math.random() * clientVersions.length)];
